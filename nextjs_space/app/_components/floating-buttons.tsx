...
export { WhatsAppIcon, TelegramIcon, YouTubeIcon, InstagramIcon, TikTokIcon };

export default function FloatingButtons() {
  const [showTop, setShowTop] = useState(false);

  useEffect(() => {
    const handler = () => setShowTop((window?.scrollY ?? 0) > 400);
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 flex flex-col gap-2.5 sm:gap-3 z-50">
      {/* WhatsApp */}
      <a
        href="https://wa.me/77072506680"
        target="_blank"
        rel="noopener noreferrer"
        className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform"
        style={{ backgroundColor: '#25D366' }}
        aria-label="WhatsApp Verumpraxis"
      >
        <WhatsAppIcon className="w-5 h-5 sm:w-6 sm:h-6" />
      </a>
      {/* Telegram */}
      <a
        href="https://t.me/verumpraxis"
        target="_blank"
        rel="noopener noreferrer"
        className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform"
        style={{ backgroundColor: '#0088cc' }}
        aria-label="Telegram Verumpraxis"
      >
      ...
      {showTop && (
        ...
        >
          ...
          <ArrowUp size={22} className="hidden sm:block" />
        </button>
      )}
    </div>
  );
